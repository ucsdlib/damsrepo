		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				...
				<style>
					...
					.propertyIcon {width:16px;background-image: url("http://library.ucsd.edu/dc/images/glyphicons-halflings-white.png");background-position:  -46px  -118px;}
		...
		<xsl:param name="propName"/>
	  	...
	  	<xsl:if test="$propNode">
		  	...
		  	<li>
			  	<table>
			  		<tr>
			  			<td class="popertyLabel_2 propertyIcon"></td>
					    <td class="popertyLabel_2"><xsl:value-of select="$propLabel"/></td>
					    <td class="popertyValue"><xsl:value-of select="$propValue" disable-output-escaping="yes"/></td>
					    <!-- 
					    <td class="damsPath">
					    	<xsl:choose>
					    		<xsl:when test="$parentPath"><xsl:value-of select="substring-after($xPath, concat($parentPath,'/'))"/></xsl:when>

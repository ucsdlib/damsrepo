...
echo "$URL/api/objects/$REP_ARK?adds=..."
...
echo "$URL/api/objects/$COL_ARK?adds=..."
...
for i in 1 2; do
	...
	echo "$URL/api/objects/$OBJ_ARK?adds=..."
	curl -u $USER:$PASS -f -X POST "$URL/api/objects/$OBJ_ARK?adds=$JSON"
	if [ $? != 0 ]; then
		ERRORS=$(( $ERRORS + 1 ))
	fi
	echo
	...
	FILE=$BASE/../sample/files/20775-bb01010101-1-1.tif
	SRCPATH=`dirname $FILE`
	echo $FILE
	echo "$URL/api/files/$OBJ_ARK/1/1.tif"
	curl -u $USER:$PASS -f -i -X POST -F sourcePath="$SRCPATH" -F file=@$FILE $URL/api/files/$OBJ_ARK/1/1.tif?fs=$FS
	if [ $? != 0 ]; then
		ERRORS=$(( $ERRORS + 1 ))
	fi
	echo
	...
	echo $FILE
	echo "$URL/api/files/$OBJ_ARK/2/1.jpg"
	curl -u $USER:$PASS -f -i -X POST -F local=$FILE $URL/api/files/$OBJ_ARK/2/1.jpg?fs=$FS
	if [ $? != 0 ]; then
		ERRORS=$(( $ERRORS + 1 ))
	fi
	echo
	...
	curl -u $USER:$PASS -f -i -L -X POST $SIZES $URL/api/files/$OBJ_ARK/1/1.tif/derivatives?fs=$FS
	if [ $? != 0 ]; then
		ERRORS=$(( $ERRORS + 1 ))
	fi
	echo

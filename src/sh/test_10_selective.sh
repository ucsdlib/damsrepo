...
curl -f -X PUT -F mode=add -F file=@tmp/tmp3.rdf.xml http://localhost:8080/dams/api/objects/$OBJ
if [ $? != 0 ]; then
    ERRORS=$(( $ERRORS + 1 ))
fi
echo
...
curl -f -X DELETE "http://localhost:8080/dams/api/objects/$OBJ/1/selective?predicate=dams%3Anote"
if [ $? != 0 ]; then
	ERRORS=$(( $ERRORS + 1 ))
fi
echo
...
curl -f -X PUT -F mode=add -F file=@tmp/tmp4.rdf.xml http://localhost:8080/dams/api/objects/$OBJ
if [ $? != 0 ]; then
    ERRORS=$(( $ERRORS + 1 ))
fi
echo
